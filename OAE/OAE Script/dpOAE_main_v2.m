...
% Updates on 12/16/2025 by SH: 
% This script is now updated to fix the ramping issue discovered 11/2025
% and to add a new amplitude and noise floor calculation
% and to save reach trial of raw data for additional analyses. 
% Originally based on code found in DPOAEgrowth.m from Hari:
...
for i = 1:length(f2)
    ...
    while(stillCollect && (trial_f < trials_max))
        ...
        % 12/2/25: SH changed from OAEI.StartTrial to OAEI.StartTrial_v2
        % (has correct ramping)
        OAEI.StartTrial_v2(f1(i),f2(i),mV_amp,round(fs*dur),trials_min);
    ...
    raw_responses(:,:,i) = OAE_response; 
    oae_data.raw_responses = raw_responses; 
